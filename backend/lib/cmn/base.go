...
func PasswordEncryption(password string) string {
	...
	if err != nil {
		return LegacyPasswordEncryption(password)
	}
...
func IsBcryptHash(encrypted string) bool {
	return strings.HasPrefix(encrypted, "$2a$") || strings.HasPrefix(encrypted, "$2b$") || strings.HasPrefix(encrypted, "$2y$")
}

func VerifyPassword(encrypted, password string) bool {
	if encrypted == "" {
		return password == ""
	}
	if IsBcryptHash(encrypted) {
		return bcrypt.CompareHashAndPassword([]byte(encrypted), []byte(password)) == nil
	}
	return encrypted == LegacyPasswordEncryption(password)
}
