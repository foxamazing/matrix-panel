...
import (
	...
	"matrix-panel/api/api_v1/common/apiData/panelApiStructs"
...
func (a *ItemIcon) SaveSort(c *gin.Context) {
	...
	transactionErr := global.Db.Transaction(func(tx *gorm.DB) error {
		// 在事务中执行一些 db 操作(从这里开始您应该使用 'tx' 而不是 'db')
...
func (a *ItemIcon) GetSiteFavicon(c *gin.Context) {
	...
	baseUrl, err := siteFavicon.NormalizeAndValidatePublicHTTPURL(req.Url)
	if err != nil {
		apiReturn.ErrorParamFomat(c, "invalid url")
		return
	}
	fullUrl := ""
	if iconUrl, err := siteFavicon.GetOneFaviconURL(baseUrl); err != nil {
	...
	if strings.HasPrefix(fullUrl, "//") {
		...
		fullUrl = protocol + "://" + strings.TrimPrefix(fullUrl, "//")
	}
	// 去除图标的get参数
	{
