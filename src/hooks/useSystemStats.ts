...
export const useSystemStats = () => {
    ...
    const fetchStats = useCallback(async () => {
        ...
        try {
            ...
            let power = 0;
            if (typeof powerData === 'number') {
                power = powerData;
            } else if (powerData && typeof powerData.total === 'number') {
                power = powerData.total;
            } else if (powerData && typeof powerData.value === 'number') {
                power = powerData.value;
            } else if (powerData && typeof powerData.power === 'number') {
                power = powerData.power;
            } else if (cpuData && typeof cpuData.power === 'number') {
                power = cpuData.power;
            } else if (cpuData && typeof cpuData.packagePower === 'number') {
                power = cpuData.packagePower;
            }
