...
const MusicWidget: React.FC<MusicWidgetProps> = ({ config, onUpdate, playRequest, isDarkMode, themeColor }) => {
    ...
    const subTextColor = 'text-[var(--text-secondary)]';
    const activeColor = `text-theme`;
    const activeBg = `bg-theme`;
    ...
    const renderBottomTools = () => (
        <div className="flex items-center gap-1 md:gap-3 relative">
            <div className="relative">
                ...
                {showQualityMenu && (
                    <div className={`absolute bottom-full left-0 mb-3 w-32 rounded-xl shadow-xl overflow-hidden animate-fade-in z-50 backdrop-blur-xl border border-[var(--glass-border)] bg-[var(--glass-bg-base)]`}>
    ...
    const renderEqualizer = () => (
        ...
        >
            <div
                className={`w-[90%] max-w-md p-6 rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-bg-base)] shadow-2xl flex flex-col gap-4`}
            ...
            >
                ...
                <div className="flex justify-center pt-2">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium bg-[var(--glass-bg-hover)] ${textColor}`}
                    >
            ...
            {lyrics.length > 0 ? (
                <div ref={lyricsContainerRef} className="w-full h-full overflow-y-auto [&::-webkit-scrollbar]:hidden text-center lyrics-mask">
                    ...
                    {lyrics.map((line, i) => {
                        ...
                        return (
                            <div key={i}
                                className={`py-4 transition-all duration-500 cursor-pointer origin-center font-bold leading-relaxed hover:opacity-100 flex flex-col items-center ${isActive ? 'text-[var(--text-primary)] scale-105' : 'text-[var(--text-muted)] blur-[1px]'}`}
                                style={{
                ...
                <div className={`flex flex-col items-center justify-center h-full opacity-50 ${textColor}`}>
                    {isResolving ? (
                        <>
                            <Loader2 className="w-12 h-12 animate-spin mb-4 text-theme" />
        ...
        <div className={`flex-1 overflow-y-auto scrollbar-thin pr-2 ${isDarkMode ? 'scrollbar-thumb-white/20' : 'scrollbar-thumb-black/20'}`}>
            {playlist.map((t, i) => (
                <div key={i} onClick={() => { setCurrentTrackIndex(i); setIsPlaying(true); }}
                    className={`flex items-center gap-4 p-3 rounded-xl cursor-pointer transition-all group mb-2 ${i === currentTrackIndex ? 'bg-[var(--glass-bg-hover)] border border-[var(--glass-border)]' : 'hover:bg-white/5 border border-transparent'}`}>
                    <div className="w-12 h-12 rounded-lg bg-[var(--glass-bg-base)] overflow-hidden shrink-0 shadow-sm relative">
                    ...
                    <button onClick={(e) => removeTrack(e, i)} className={`p-2 ${subTextColor} hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity`}><Trash2 className="w-5 h-5" /></button>
                </div>
            ))}
        </div>
    );
    ...
    const overlayColor = isDarkMode ? 'bg-black/70' : 'bg-white/70';
    const fullscreenClass = `fixed inset-0 z-[60] flex flex-col overflow-hidden select-none font-sans bg-zinc-50 dark:bg-[#0a0a0a] transition-all duration-300 origin-bottom ${isClosing ? 'opacity-0 scale-75 translate-y-[40vh] rounded-[3rem]' : 'opacity-100 scale-100 translate-y-0 animate-fade-in'}`;
            ...
            {isFullscreen && (
                <div className={fullscreenClass}>
                    ...
                    <div className="absolute inset-0 z-0 pointer-events-none">
                        <div className={`absolute inset-0 transition-colors duration-1000 ${isDarkMode ? 'bg-[#0a0a0a]' : 'bg-zinc-50'}`} />
                        {displayCover && (
                            <>
                                <div
                            ...
                            <div className="flex flex-col h-full animate-fade-in">
                                        ...
                                        >
                                            ...
                                            <div className="w-[65%] h-[65%] rounded-full overflow-hidden border border-[#2a2a2a] relative z-10 shadow-inner bg-black">
                                                {displayCover ? (
                                                    ...
                                                    <div className="w-full h-full flex items-center justify-center bg-[var(--glass-bg-base)]"><Disc className="w-10 h-10 text-[var(--text-muted)] opacity-30" /></div>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="px-6 pb-4 flex flex-col gap-2">
                                    <div className="flex items-center gap-3 text-xs font-mono font-bold w-full">
                                        ...
                                        <div className="flex-1 relative h-1.5 bg-[var(--glass-border)] rounded-full cursor-pointer group overflow-hidden"
                                            onClick={(e) => {
                                                if (audioRef.current) {
                                                    const rect = e.currentTarget.getBoundingClientRect();
                                    ...
                                    <div className="flex items-center justify-between px-2">
                                        ...
                                        <button onClick={(e) => togglePlay(e)} className={`w-12 h-12 rounded-full flex items-center justify-center shadow-xl shadow-theme/30 cursor-pointer active:scale-95 bg-theme text-white`}>
                            ...
                            <div className="flex flex-col h-full animate-fade-in relative">
                                ...
                                <div className={`p-4 pb-4 flex items-center justify-between`}>
                                    ...
                                    <div className="flex items-center gap-8">
                                        ...
                                        <button onClick={(e) => togglePlay(e)} className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg shadow-theme/30 active:scale-95 bg-theme text-white`}>
                                ...
                                >
                                    ...
                                    <div className="w-[65%] h-[65%] rounded-full overflow-hidden border border-[#2a2a2a] relative z-10 shadow-inner bg-black">
                                        {displayCover ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center bg-[var(--glass-bg-base)]">
                                                <Disc className="w-16 h-16 text-[var(--text-muted)] opacity-30" />
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        <div className="max-w-7xl mx-auto flex flex-col gap-3">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center justify-center gap-6 flex-1">
                                    ...
                                    <button onClick={(e) => togglePlay(e)} className={`w-14 h-14 rounded-full flex items-center justify-center shadow-xl shadow-theme/30 cursor-pointer transition-transform hover:scale-105 active:scale-95 bg-theme text-white hover:opacity-90`}>
                                        {isResolving ? <Loader2 className="w-6 h-6 animate-spin" /> : (isPlaying ? <Pause className="w-6 h-6 fill-current" /> : <Play className="w-6 h-6 fill-current ml-1" />)}
                                    </button>
        ...
        `}
            ...
            >
                <div className={`pointer-events-auto relative group ${viewMode === 'mini' ? 'cursor-grab active:cursor-grabbing' : ''}`}>
                    <div
                        className={`glass-panel transition-all duration-[600ms] ease-spring-bouncy flex items-center overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.4)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.8)]
               ${viewMode === 'compact' ? 'w-[92vw] md:w-[320px] h-[56px] rounded-3xl px-3 ring-1 ring-white/20 dark:ring-white/10' : 'w-16 h-16 rounded-full hover:scale-110 hover:shadow-[0_15px_40px_rgba(0,0,0,0.5)] ring-1 ring-white/30 dark:ring-white/10 block'}
            ...
            `}
                    >
                        {viewMode === 'mini' ? (
                            ...
                            <>
                                <div className="w-[40px] h-[40px] rounded-full overflow-hidden shrink-0 relative group/cover cursor-pointer shadow-lg my-auto ring-1 ring-white/20 dark:ring-white/5" onClick={() => {
                                    setShowPlaylist(false);
                                    ...
                                    {displayCover ? (
                                        ...
                                        <div className={`w-full h-full flex items-center justify-center bg-[var(--glass-bg-base)]`}><Music className={`w-6 h-6 text-[var(--text-muted)]`} /></div>
                                    )}
                                ...
                                <div className="flex-1 min-w-0 mx-2.5 flex flex-col justify-center h-full cursor-default" onClick={() => setViewMode('fullscreen')}>
                                    <div className={`text-[13px] font-bold truncate leading-tight tracking-wide ${textColor}`}>{currentTrack?.name || 'Not Playing'}</div>
                                ...
                                <div className="flex items-center gap-1">
                                    <button onClick={handlePrev} className={`hidden md:block p-1.5 transition-colors cursor-pointer rounded-full hover:bg-current/10 ${textColor}`}><SkipBack className="w-4 h-4 fill-current" /></button>
                                    <button onClick={(e) => togglePlay(e)} className={`p-2 hover:scale-110 transition-transform cursor-pointer rounded-full ${activeColor}`}>
                                        {isResolving ? <Loader2 className="w-5 h-5 animate-spin" /> : (isPlaying ? <Pause className="w-6 h-6 fill-current" /> : <Play className="w-6 h-6 fill-current ml-0.5" />)}
                                    </button>
                                    <button onClick={() => handleNext(false)} className={`p-1.5 transition-colors cursor-pointer rounded-full hover:bg-current/10 ${textColor}`}><SkipForward className="w-4 h-4 fill-current" /></button>
                                </div>

                                <div className={`hidden md:block w-px h-7 mx-2.5 ${isDarkMode ? 'bg-white/10' : 'bg-black/10'}`}></div>

                                <button
                                    onClick={() => setViewMode('mini')}
                                    className={`p-2.5 cursor-pointer rounded-full hover:bg-current/10 transition-colors ${subTextColor} hover:${textColor}`}
                                    title="最小化到悬浮球"
                                >
